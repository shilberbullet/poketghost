...
float getTypeEffectiveness(YokaiType moveType, YokaiType defenderType) {
    ...
    switch (moveType) {
        case TYPE_EVIL_SPIRIT:
            if (defenderType == TYPE_GHOST) return 2.0f;  // 악귀 기술은 원귀에게 효과적
            if (defenderType == TYPE_HUMAN || defenderType == TYPE_MONSTER) return 0.5f;  // 악귀 기술은 인간형과 괴수형에게 약함
            break;
        case TYPE_GHOST:
            if (defenderType == TYPE_HUMAN || defenderType == TYPE_MONSTER) return 2.0f;  // 원귀 기술은 인간형과 괴수형에게 효과적
            if (defenderType == TYPE_EVIL_SPIRIT) return 0.5f;  // 원귀 기술은 악귀에게 약함
            break;
        case TYPE_HUMAN:
            if (defenderType == TYPE_EVIL_SPIRIT) return 2.0f;  // 인간형 기술은 악귀에게 효과적
            if (defenderType == TYPE_GHOST || defenderType == TYPE_ANIMAL) return 0.5f;  // 인간형 기술은 원귀와 동물형에게 약함
            break;
        case TYPE_ANIMAL:
            if (defenderType == TYPE_HUMAN) return 2.0f;  // 동물형 기술은 인간형에게 효과적
            if (defenderType == TYPE_MONSTER) return 0.5f;  // 동물형 기술은 괴수형에게 약함
            break;
        case TYPE_MONSTER:
            if (defenderType == TYPE_EVIL_SPIRIT || defenderType == TYPE_ANIMAL) return 2.0f;  // 괴수형 기술은 악귀와 동물형에게 효과적
            if (defenderType == TYPE_GHOST) return 0.5f;  // 괴수형 기술은 원귀에게 약함
            break;
    }
    return 1.0f;  // 기본 상성
}
